...
4. *Function parameter names in calls*. This is useful, but can be a bit annoying,
   specially considering that, as far as I know, it can't be directly configured
   from Eglot. If this was an option, and it wasn't on by default, I honestly
   would not bother turning it on.
