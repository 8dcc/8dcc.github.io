...
1. It's not multiplying the index by the size of the elements (e.g. =i*4= for
   arrays of DWORDs), so we can asume the size of each element is one byte.
2. Right after =.L8=, you can see it's moving and comparing byte pointers, so it's
...
We can see that they are =for= loops because it's initializing a variable right
before labels (=.L4= and =.L8=) that will be used for iteration.

I found that a good approach is to look at the places where those labels are
being used, and get the loop condition from there.

** Conditional move

After =.L8=, it compares =rax[rsi]= with =rdx[rsi]=, and moves =r9d= to =r8d= if they are
not equal. To do this, it uses the =cmovne= instruction.
