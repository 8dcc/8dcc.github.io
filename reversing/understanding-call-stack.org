...
necessarily talking about our C object files, but also our hand-written assembly
...
we need to call =printf= through the [[https://en.wikipedia.org/wiki/Position-independent_code#Technical_details][Procedure Linkage Table]] (PLT) instead of a
direct call. That's exactly what =wrt ..plt= is doing.
...
until run time, and the dynamic linker has to fill it out. The linker doesn't
like runtime relocations in =.text=, hence the "relocation in read-only section
=.text=" warning. Since =printf= needs an absolute address, we can convert the
RIP-relative address to an absolute one with =lea rdi, [rel fmt]=. The =rel= stands
for relative, and although we don't need it since we specified =default rel= at
the top, I chose to emphasize it anyway.
