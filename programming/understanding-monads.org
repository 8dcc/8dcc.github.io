...
the =bind= function can be implemented, which receives a =LoggedInt= value (=a=), one
of the arithmetic functions, and a simple integer (=b=). It performs the following
steps:
...
- Left identity ::
  ...
  Promoting a simple value into a the monadic type through the constructor, and
