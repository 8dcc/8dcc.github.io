...
These are the general steps for establishing a passive connection:
...
This is the new code for obtaining our address information, without error
checking. In this case, the returned =addrinfo= structure will refer to the port
=4321= of our machine.
...
getaddrinfo(NULL, "4321", &hints, &self_info); /* Updated */
#+end_src
...
=socket= just like we did [[#obtaining-the-socket-descriptor][before]]. Again, without error checking:
