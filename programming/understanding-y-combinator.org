...
all examples in both lambda notation, Scheme and JavaScript.
...
first is applied to the second one. Let's try to simplify it by \beta-reduction.

[[file:../img/ycombinator2.png]]

We are right back where we started. This function would call itself
indefinitely, and a similar form will be used for the Y combinator bellow. But
first, we must learn about fixed points.
