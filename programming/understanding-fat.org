...
the actual contents of the file are stored in one or more clusters in the /data
region/ (located after the root directory, as shown in [[fig1][Figure 1]]). The purpose of
the FAT is to store, for each file, a list of the cluster numbers that hold its
data.
...
The size of each FAT entry changes depending on the filesystem version, in fact,
the 12/16/32 number indicates the size of a FAT entry in bits.

When explaining the layout of the FAT, I will use the 16-bit version, since each
entry is two bytes, and therefore easier to understand. The 12-bit version,
which uses one bit and a half (i.e. 3 [[https://en.wikipedia.org/wiki/Nibble][nibbles]]), is a bit more tricky, so it will
