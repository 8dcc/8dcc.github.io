...
value bigger or equal than ~0xFFF8~, for FAT16).

For example, using the cluster indexes from the last figure, the contents of the
FAT could be described using the following pseudo-code.
...
not the first one. Either way, the FAT is used to keep track of the clusters
that store the contents of files, by building linked lists with their cluster
indexes.
