...
for pool allocation, [[https://github.com/8dcc/libpool][libpool]]. Some new features, as well as fixes, have been
added to that repository, and they might not be reflected here. These new
features include thread safety, pointer alignment, logging and support for
non-POSIX systems, so feel free to check it out as well.
