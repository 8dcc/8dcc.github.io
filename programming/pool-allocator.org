...
library, with some notable "visual" differences explained immediately. Some
improvements, like being able to resize/reallocate an existing pool, will also
be expanded [[#second-implementation-resizing-the-pool][below]].
...
** COMMENT Changes to =pool_new= and =pool_close=
:PROPERTIES:
